...
import { useOrbitalObjects } from '../services/objectService'
...
export default function TrackPage() {
  ...
  useEffect(() => {
    if (!selectedObject && orbitalObjects.length > 0) {
  ...
  const filteredObjects = useMemo(() => {
    ...
    return orbitalObjects.filter((object) => {
      const matchesSearch =
        !query ||
  ...
  }, [search, riskFilter, orbitalObjects])

  const riskCounts = useMemo(() => {
    return {
      total: orbitalObjects.length,
      normal: orbitalObjects.filter(
        (object) => object.risk === 'normal',
      ).length,
      elevated: orbitalObjects.filter(
        (object) => object.risk === 'elevated',
      ).length,
      critical: orbitalObjects.filter(
        ...
        <div className="object-registry">
          <div className="registry-header">
            ...
            <span className="registry-count">
              {filteredObjects.length} / {orbitalObjects.length}
            </span>
          </div>
